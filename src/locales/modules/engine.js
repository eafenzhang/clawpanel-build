...
export default {
  ...
  dashNativePanelWindowsDesc: _('Hermes Agent 在原生 Windows 上无条件导入 POSIX-only 标准库（fcntl / termios），导致 dashboard 启动崩溃。这是已知的上游 bug，官方暂未修复。',
    ...
    'Hermes Agent 在原生 Windows 上無條件匯入 POSIX-only 標準函式庫（fcntl / termios），導致 dashboard 啟動崩潰。這是已知的上游 bug，官方暫未修復。',
    'Hermes Agent はネイティブ Windows で POSIX 専用の標準ライブラリ（fcntl / termios）を無条件にインポートするため、dashboard が起動時にクラッシュします。これは既知のアップストリームバグで未修正です。',
  ...
  dashNativePanelDepHint: _('首次使用需要先安装 Web 依赖（fastapi + uvicorn）。点击下方按钮自动安装，或在终端运行：',
    ...
    '首次使用需要先安裝 Web 依賴（fastapi + uvicorn）。點擊下方按鈕自動安裝，或在終端執行：',
    '初回使用には Web 依存関係（fastapi + uvicorn）のインストールが必要です。下のボタンで自動インストールするか、ターミナルで実行：',
